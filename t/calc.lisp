...
(deftest.ps+ for-coordinate-functions
  ...
  (testing "calc-parent-global-point"
    (let ((grand-parent (make-ecs-entity))
          (parent (make-ecs-entity))
          (stranger (make-ecs-entity))
          (child (make-ecs-entity)))
      ...
      (setf (ecs-entity-parent stranger) parent)
      (setf (ecs-entity-parent parent) grand-parent)
      (let ((global-pnt (calc-parent-global-point child)))
        (ok (is-point global-pnt 1 1 (* PI -2/2))))
      (let ((global-pnt (calc-parent-global-point grand-parent)))
        (ok (is-point global-pnt 0 0 0)))
      (let* ((offset (make-point-2d :x 0 :y 1 :angle (* PI -1/2)))
             (result (calc-parent-global-point child offset)))
        (ok (is-point result 1 0 (* PI -3/2))))))
